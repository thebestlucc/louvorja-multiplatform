...
export async function clearActivePlayback(): Promise<void> {
  ...
  const { resetCoordinatorPlaybackState } =
    await import("../hooks/use-playback-coordinator");
  ...
  ensureProjectionPromise = (async () => {
    const [_, error] = await catcher(
      async () => {
        ...
        const resolvedIndexes = resolveProjectionMonitorIndexes(
          monitors,
          monitorConfigs,
        );
        if (!resolvedIndexes) {
...
export async function projectSlideIndex(index: number): Promise<void> {
  ...
  const projectionType: "hymn" | "presentation" = currentPresentationId
    ? "presentation"
    : "hymn";
