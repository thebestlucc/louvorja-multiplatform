...
#[tauri::command]
pub fn search_hymns(query: String, state: tauri::State<'_, AppState>) -> Result<Vec<Hymn>, AppError> {
...
#[tauri::command]
pub fn get_hymns_by_album(album: String, state: tauri::State<'_, AppState>) -> Result<Vec<Hymn>, AppError> {
    let conn = state.db.lock().map_err(|e| AppError::Internal(e.to_string()))?;
