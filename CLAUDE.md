...
- **Pack sync disk I/O pattern (low-end devices):** Background thread priority (`THREAD_MODE_BACKGROUND_BEGIN`), 3 max concurrent downloads, `BufWriter::with_capacity(256*1024)`, `FILE_FLAG_SEQUENTIAL_SCAN`, 64MB mmap. FTS5 init + OPTIMIZE run in a **background thread** (`FTS_HANDLES` static in `executor.rs`) so sync completes as soon as pool is live — hymn list works immediately, FTS search ready shortly after (`fts-ready` event). `synchronous=OFF` + `temp_store=MEMORY` on the dedicated FTS connection eliminate WAL fsyncs during bulk INSERT + OPTIMIZE. Before file rename, any previous FTS background thread is joined (prevents Windows file-in-use on consecutive syncs). See `pack_sync/executor.rs`.
